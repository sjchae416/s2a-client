...
export default function ViewConfig({
  ...
  useEffect(() => {
    if (selectedView.viewName) {
      ...
      setSelectedTable(selectedView.selectedTable);

      const res = tables.find(
        (item) => item?._id === selectedView.selectedTable
      )?.columns;
    ...
    >
      ...
      <div className="form-group">
        ...
        <select onChange={(e) => handleSelectTable(e)} className="form-control">
          ...
          {tables.map((table) => (
            <option
              selected={selectedView.selectedTable === table._id ? true : false}
              key={table._id}
              value={table._id}
            >
              {table.name}
