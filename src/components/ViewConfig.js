...
export default function ViewConfig({
  ...
  const handleUpdateView = async () => {
    if (!viewName) {
      ...
      return window.alert("Choose role!");
    } else if(viewType === "Detail" && !selectedView.editableCols.includes(keyCol)){
		  return window.alert("Key column " + keyCol + " must be editable");
	}else {
	...
	const handleEditFilterButtonChange = (e, name) => {
		console.log('🚀 ~ handleEditFilterButtonChange ~ name:', name);
		// console.log(name);
		setEditFilter(name);
		// console.log(viewData);
	};
						...
						{boolConfigs.map((config) => (
							<div key={config.name}>
								<input
									...
									value={config.name}
									// FIXME not storing properly! not called at all!
